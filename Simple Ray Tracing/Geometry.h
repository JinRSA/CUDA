...
template <typename T> struct vec<2, T>
{
	...
	template <class U>
	__host__ __device__ vec<2, T>(const vec<2, U>& v);
...
template <typename T> struct vec<3, T>
{
	...
	__host__ __device__ vec<3, T>& normalize(T l = 1) { *this = (*this) * (l / norm()); return *this; }	// *this *= ?!
	T x, y, z;
};
...
__host__ __device__ vec<DIM, T> operator*(const vec<DIM, T>& lhs, const U& rhs)
{
	vec<DIM, T> ret;
...
template <size_t DIM, typename T>
__host__ __device__ std::ostream& operator<<(std::ostream& out, const vec<DIM, T>& v)
{
